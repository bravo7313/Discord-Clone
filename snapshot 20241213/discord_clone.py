...
class App(ctk.CTk):
    def __init__(self):
        ...
        global fonts, images, login_name_entry, login_password_entry
        ...
        login_name_label=ctk.CTkLabel(master=main_frame, text="UserName:")
        login_name_label.grid(row=2,column=0,pady=(40,5),padx=(50,0))

        login_name_entry=ctk.CTkEntry(master=main_frame, placeholder_text="UserName", width=(screensize[0]/3)-100,height=30, corner_radius=10)
        ...
        login_button.grid(row=5,column=1,pady=35,padx=(0,50),columnspan=3,sticky='we')
        ...
        forget_password_button.grid(row=6,column=1,padx=(0,50),columnspan=3,sticky='we')
        ...
        register_button.grid(row=7,column=1,padx=(0,50),columnspan=3,sticky='we')
    ...
    def login_func(self):
        ...
        user_password=login_password_entry.get()
    ...
    def mail_check_func(self):
        ...
        user_id_list=[]
        templist=[]
        with open(r"database\login_data.csv",'r') as csvfile:
            ...
            if not user_email.lower()in user_email_list:
            ...
            with open(r"database\login_data.csv",'r') as csvfile:
                ...
                for i in range(len(user_id_list)):
                    if user_email==user_id_list[i][0]:                            
                        user_id_list[i][1]=new_password
            with open(r"database\login_data.csv",'w') as csvfile:
                csvwriter=csv.writer(csvfile)
                csvwriter.writerows(user_id_list)
    ...
    def database_check_func(self):
        if not os.path.isfile(r"database\login_data.csv"):
            with open(os.path.join("database\\","login_data.csv"),'w')as fd:
                pass
    ...
    def register_func(self):
        ...
        for i in range(1,13):
            month.append(str(i))
        for i in range(1800,int(current_date.strftime("%Y"))):
            year.append(str(i))
        
        date_of_birth_label=ctk.CTkLabel(master=register_frame, text="Date of Birth")
        date_of_birth_label.place(x=50,y=370)
        day_combo=ctk.CTkComboBox(master=register_frame, values=day,width=100,justify='center')
        day_combo.place(x=63,y=400)
        month_combo=ctk.CTkComboBox(master=register_frame, values=month,width=100,justify='center')
        month_combo.place(x=189,y=400)
        year_combo=ctk.CTkComboBox(master=register_frame, values=year,width=100,justify='center')
    ...
    def register_comfirm_func(self):
        ...
        with open(r"database\login_data.csv",'r') as csvfile:
                ...
                for i in range(len(user_id_list)):
                    user_email_list.append(user_id_list[i][0])
                    user_name_list.append(user_id_list[i][2])
